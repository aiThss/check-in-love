...
// ── Reaction Bar ──────────────────────────────────────────────────────────────
function buildReactionBar(checkin, onReact) {
    const bar = document.createElement('div');
    bar.className = 'reaction-bar';
    bar.style.cssText = 'padding:12px 4px;gap:8px;flex-wrap:nowrap;overflow-x:auto;';
    REACTIONS.forEach((emoji) => {
        const existing = checkin.reactions.find((r) => r.type === emoji);
...
// ── Check-in Card ─────────────────────────────────────────────────────────────
function buildCheckinCard(checkin, onReact) {
    const wrapper = document.createElement('div');
...
// ── Empty State ───────────────────────────────────────────────────────────────
function buildEmptyState(partnerName) {
      ...
      `;
            const cardEl = buildCheckinCard(checkin, async (type) => {
                try {
                    ...
                    if (bar) {
                        REACTIONS.forEach((emoji) => {
                            const btn = bar.querySelector(`[data-type="${emoji}"]`);
                            if (!btn)
                                return;
                            const r = updatedReactions.find((rx) => rx.type === emoji);
